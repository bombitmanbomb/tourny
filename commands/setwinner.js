exports.run = (client, message, args) => {
    try {
        ...
        if (args.length !== 1) {
            message.channel.send("Usage: " + config.prefix + "setwinner <user>").then(warn => warn.delete(4000))
            message.delete()
            return
        }
        ...
        tDat[index].winner = args[0]
        db.set("tournaments".tDat).write()
        message.reply("Winner Set").then(warn => warn.delete(4000))
        message.delete()
    }
    catch (err) {
        console.log(err)
