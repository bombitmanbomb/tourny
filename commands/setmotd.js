exports.run = (client, message, args) => {
    try {
        ...
        if (args.length < 1) {
            message.channel.send("Usage: " + config.prefix + "setmotd <message> (Discord Formatting is Supported.)").then(warn => warn.delete(4000))
            message.delete()
            return
        }
        ...
        tDat[index].motd = args.join(" ")
        db.set("tournaments".tDat).write()
        message.reply("MOTD Set").then(warn => warn.delete(4000))
        message.delete()
    }
    catch (err) {
        console.log(err)
